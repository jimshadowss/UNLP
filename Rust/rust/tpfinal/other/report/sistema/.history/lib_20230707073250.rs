...
mod sistema {
            ...
            if !esta {
                match InfoCat::new(cat, self, actividad) {
                    Ok(a) => {
                        ...
                        let pago = Pago::new(precio, socio.clone(), self.env().block_timestamp());
    ...
    mod tests {
        ...
        fn new_a_o_c_test3() {
            let mut sis = Sistema::default();
            sis.precio_a = 30;
            let info = InfoCat::new('c', &sis, None).unwrap();
        ...
        fn new_b_test() {
            let mut sis = Sistema::default();
            sis.precio_b = 30;
            let info = InfoCat::new('b', &sis, Some(Actividades::default())).unwrap();
            assert!(info.precio_actual == sis.precio_b && info.actividades.len() == 1);
        }
        #[ink::test]
        fn new_b_test2() {
            let sis = Sistema::default();
            let res = InfoCat::new('a', &sis, Some(Actividades::default()));
            assert!(res==Err("Las entradas validas son A y C sin opcion de actividad, y B con opcion de actividad".to_string()))
        }
        #[ink::test]
