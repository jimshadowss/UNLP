...
mod sistema {
    ...
    impl Sistema {
                    ...
                    if !esta {
                        match InfoCat::new(cat.clone(), self, actividad) {
                            Ok(a) => {
                                ...
                                pago =
                                    Pago::new(precio, socio.clone(), self.env().block_timestamp());
        ...
        #[ink(message)]
        pub fn get_adress(&self) -> AccountId {
            self.env().caller()
        }
        #[ink(message)]
    ...
    mod tests {
        ...
        fn agregar_socio_test6() {
            ...
            assert!(
                sis.registro_pagos[0].vencimiento >= sis.env().block_timestamp() + 864000000
                    && sis.registro_pagos[0].vencimiento
                        < sis.env().block_timestamp() + 864060000
            )
        }
        #[ink::test]
        fn get_socio_test() {
        ...
        fn registrar_pago_test6(){
                ...
                sis.agregar_socio2(dni, nombre.clone(), categoria.clone(), actividad.clone())
            {
                sis.set_precio_a(2000);
                if let Ok(_) = sis.registrar_pago(dni, 2000){
                    assert!(sis.registro_pagos[1].vencimiento==sis.registro_pagos[0].vencimiento+2_592_000_000)
                }
        }
        #[ink::test]
        ...
        // #[ink::test]
        // fn ver_diferencias_de_fechas_agregar_pago_y_agregar_socio()
    }
}
