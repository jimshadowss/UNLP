    ...
    mod tests {
        ...
        fn registrar_pago_test6(){
                ...
                sis.agregar_socio2(dni, nombre.clone(), categoria.clone(), actividad.clone())
            {
                sis.set_precio_a(2000);
                if let Ok(_) = sis.registrar_pago(dni, 2000){
                    assert!(sis.registro_pagos[1].vencimiento>=sis.registro_pagos[0].vencimiento+2_592_000_000&&sis.registro_pagos[1].vencimiento<sis.registro_pagos[0].vencimiento+2_592_060_000)
                }
        }
        #[ink::test]
