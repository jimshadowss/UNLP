...
mod sistema {
    ...
    mod tests {
        ...
        fn agregar_socio_test2() {
            ...
            {
                res = sis.agregar_socio2(dni, nombre.clone(), categoria, actividad);
            }

            assert!(res == Err("Este socio ya existe".to_string()))
        }
        #[ink::test]
        ...
        fn consulta_pagos_test2() {
            ...
            sis.registrar_pago(1, sis.precio_a);
            let vec = sis.consulta_pagos(None);
            let mut esta_solo = true;
            for i in &vec {
                if i.socio.dni != 1 {
                    esta_solo = false;
