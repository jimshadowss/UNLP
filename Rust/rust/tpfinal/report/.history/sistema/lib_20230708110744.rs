...
mod sistema {
    ...
    impl Pago {
        ...
        ///para uso interno, checkea si el pago está fuera de termino
        fn fuera_de_termino(&self, sis: &Sistema) -> bool {
            if (!self.pagado && sis.env().block_timestamp() > self.vencimiento)
