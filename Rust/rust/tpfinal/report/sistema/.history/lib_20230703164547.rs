...
mod sistema {
    ...
    impl Sistema {
        ...
        fn registrar_pago2(&mut self, dni: u32, monto_pagado: u32) -> bool {
            ...
            if let Some(a) = self.get_socio(dni) {
                for i in 0..self.registro_pagos.len() {
                    if a.dni == self.registro_pagos[i].socio.dni
