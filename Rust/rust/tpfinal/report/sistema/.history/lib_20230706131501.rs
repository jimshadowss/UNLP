...
mod sistema {
            ...
            match cat {
                'B' | 'b' => {
                    if let Some(a)=actividad{
                        ...
                        Err("Las entradas validas son A y C sin opcion de actividad, y B con opcion de actividad".to_string())
                }
                }
                _ => Err("Las entradas validas son A y C sin opcion de actividad, y B con opcion de actividad".to_string()),
            }
        ...
        fn agregar_socio2(
            ...
            if !esta {
                if (cat.eq(&"A".to_string()) || cat.eq(&"a".to_string())) && actividad == None {
                    ...
                    match InfoCat::new_b('B', self, actividad) {
                        Ok(a) => {
                            info = a;
                            res = Ok("Usuario agregado correctamente".to_string());
    ...
    mod tests {
        ...
        fn new_b_test() {
            let mut sis = Sistema::default();
            sis.precio_b = 30;
            let info = InfoCat::new_b('b', &sis, Some(Actividades::default())).unwrap();
            assert!(info.precio_actual == sis.precio_b && info.actividades.len() == 1);
        }
        #[ink::test]
        fn new_b_test2() {
            let sis = Sistema::default();
            let res = InfoCat::new_b('a', &sis, Some(Actividades::default()));
            assert!(res==Err("Las entradas validas son A y C sin opcion de actividad, y B con opcion de actividad".to_string()))
        }
        #[ink::test]
