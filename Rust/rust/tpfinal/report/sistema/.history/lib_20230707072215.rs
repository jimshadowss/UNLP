...
mod sistema {
    ...
    impl Sistema {
        ...
        fn agregar_socio2(
            ...
            if !esta {
                match InfoCat::new(cat, self, actividad) {
                    Ok(a) => {
                        info = a;
                        res = Ok("Usuario agregado correctamente".to_string());
                        let categ = match cat{
                            String::from('A')|String::from('a')=>Categorias::A(info),
                            String::from('B')|String::from('b')=>Categorias::B(info),
                            String::from('C')|String::from('c')=>Categorias::C(info),
                        }
