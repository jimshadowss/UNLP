...
mod sistema {
    ...
    mod tests {
        ...
        #[ink::test]
        fn new_infocat_test_6(){
            let mut sis = Sistema::default();
            let res = InfoCat::new('a'.to_string(), &sis, Some(Actividades::Futbol));
            assert!(res==Err("Las entradas validas son A y C sin opcion de actividad, y B con opcion de actividad".to_string()));
        }
        #[ink::test]
        fn new_infocat_test_7(){
            let mut sis = Sistema::default();
            let res = InfoCat::new('b'.to_string(), &sis, Some(Actividades::Hockey));
            assert!(res==Err("Las entradas validas son A y C sin opcion de actividad, y B con opcion de actividad".to_string()));
        }
        #[ink::test]
        ...
        fn agregar_socio_test() {
            let mut sis = Sistema::new(1500, 50, 800, 30, 2);
            let dni = 1;
