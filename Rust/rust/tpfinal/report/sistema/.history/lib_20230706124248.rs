...
mod sistema {
    ...
    impl InfoCat {
        fn new_a_o_c(cat: char, sistema: &Sistema) -> Result<InfoCat, String> {
            match cat {
                'A' | 'a' => {
                ...
                _ => Err("Las entradas validas son A y C sin opcion de actividad, y B con opcion de actividad".to_string()),
            }
        }
        fn new_b(cat: char, sistema: &Sistema, actividad: Actividades) -> Result<InfoCat, String> {
            match cat {
                'B' | 'b' => {
                    let mut actividades = Vec::new();
    ...
    impl Sistema {
        ...
        fn agregar_socio2(
            ...
            let socio: Socio;
            let pago: Pago;
            let mut res = Err("No hecho".to_string());
            let mut esta = false;
            ...
            if !esta {
                if (cat.eq(&"A".to_string()) || cat.eq(&"a".to_string())) && actividad == None {
                    match InfoCat::new_a_o_c('A', self) {
                        Ok(a) => {
                            info = a;
                    ...
                    && actividad == None
                {
                    match InfoCat::new_a_o_c('C', self) {
                        Ok(a) => {
                            info = a;
                            res = Ok("Usuario agregado correctamente".to_string());
                    ...
                    res=Err("Las entradas validas son A y C sin opcion de actividad, y B con opcion de actividad".to_string());
                }
            } else {
                ...
                for i in 0..self.registro_pagos.len() {
                        ...
                        && self.registro_pagos[i].pagado == false
                    {
                            ...
                            && monto_pagado >= pago_aux.monto * self.porcentaje_descuento / 100
                        {
                            ...
                            hecho = Ok("Pago registrado exitosamente".to_string());
    ...
    mod tests {
        ...
        fn new_a_o_c_test3() {
            let mut sis = Sistema::default();
            sis.precio_a = 30;
            let info = InfoCat::new_a_o_c('c', &sis).unwrap();
        ...
        fn new_b_test() {
            let mut sis = Sistema::default();
            sis.precio_b = 30;
            let info = InfoCat::new_b('b', &sis, Actividades::default()).unwrap();
            assert!(info.precio_actual == sis.precio_b && info.actividades.len() == 1);
        }
        #[ink::test]
        fn new_b_test2() {
            let sis = Sistema::default();
            let res = InfoCat::new_b('a', &sis, Actividades::default());
            assert!(res==Err("Las entradas validas son A y C sin opcion de actividad, y B con opcion de actividad".to_string()))
        }
        #[ink::test]
        ...
        fn agregar_socio_test() {
            ...
            let actividad = None;
            let mut esta = false;
            if let Ok(_) = sis.agregar_socio2(dni, nombre.clone(), categoria.clone(), actividad) {
        ...
        fn agregar_socio_test2() {
            ...
            let res = sis.agregar_socio2(dni, nombre.clone(), categoria, actividad);
        ...
        fn registrar_pago_test() {
            ...
            if let Ok(_) =
                sis.agregar_socio2(dni, nombre.clone(), categoria.clone(), actividad.clone())
            {
                if let Ok(_) = sis.registrar_pago2(dni, sis.precio_a) {
                    venc = sis.get_proximo_pago2(dni).unwrap().vencimiento;
                    if let Ok(a) = sis.registrar_pago2(dni, sis.precio_a) {}
                }
            }
            ...
            assert!(
                sis.get_proximo_pago2(dni).unwrap().vencimiento >= venc
                    ...
                    && a == "Pago registrado exitosamente".to_string()
            )
        }
        #[ink::test]
