...
mod sistema {
    ...
    mod tests {
        ...
        fn new_pago_test() {
            ...
            let pago = Pago::new(monto, socio.clone(), vencimiento, con_descuento);
            assert!(
                pago.monto == monto
        ...
        #[ink::test]
        fn set_porcentaje_test() {
            let mut sis = Sistema::default();
            let p = 30;
            sis.set_porcentaje(p);
            assert_eq!(sis.porcentaje_descuento, p)
        }
        #[ink::test]
        fn get_porcentaje_test() {
            let mut sis = Sistema::default();
            let p = 30;
            sis.porcentaje_descuento == p;
            assert_eq!(sis.get_porcentaje(), p)
        }
        #[ink::test]
        fn set_pagos_consecutivos_test() {
            let mut sis = Sistema::default();
            let p = 30;
            sis.set_cantidad_pagos_consecutivos(p);
            assert_eq!(sis.cantidad_pagos_consecutivos, p)
        }
        #[ink::test]
        fn get_pagos_consecutivos_test() {
            let mut sis = Sistema::default();
            let p = 30;
            sis.cantidad_pagos_consecutivos == p;
            assert_eq!(sis.get_cantidad_pagos_consecutivos, p)
        }
        #[ink::test]
        fn agregar_socio_test() {
            ...
            let categoria = Categorias::default();
            let actividad = Some(Actividades::default());
            sis.agregar_socio2(dni, nombre, categoria, actividad)
        }
    }
}
